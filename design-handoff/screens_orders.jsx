...
(function () {
  ...
  const SECTION_NAMES = (window.DATA && window.DATA.SECTION_NAMES) || {};
  ...
  const otklikPlural = (n) => { const a = Math.abs(n) % 100, b = a % 10; if (a > 10 && a < 20) return "откликов"; if (b > 1 && b < 5) return "отклика"; if (b === 1) return "отклик"; return "откликов"; };
  const STAGE_OPTS = [["Эскиз", "Эскизный проект"], ["П", "Стадия «Проект»"], ["ПД", "Проектная документация"], ["РД", "Рабочая документация"]];

  function StatusBadge({ status }) {
    const s = STATUS[status];
    return <span className={"badge ostat ostat--" + s.cls}><i />{s.label}</span>;
  }
      ...
      <div className="card card-hover ordercard" onClick={() => go("order-detail", o.id)}>
        ...
        <div className="grow" style={{ minWidth: 0 }}>
          ...
          <div className="row between mt16">
            ...
            <span className={"ordercard__replies" + (o.replies >= 8 ? " is-hot" : "")}><Icon name={o.replies >= 8 ? "spark" : "comment"} size={14} /><b>{o.replies}</b> {otklikPlural(o.replies)}</span>
          </div>
        </div>
      </div>
    );
        ...
        <>
          ...
          <div className="card">
            <div className="row between" style={{ marginBottom: 16 }}>
              <h3 className="section-title" style={{ margin: 0 }}>Разделы документации</h3>
              <span className="dim" style={{ fontSize: 13 }}>{o.sections.length} {sectPlural(o.sections.length)}</span>
            </div>
            <div className="secgrid">
              {o.sections.map(s => (
                <div key={s} className="secitem">
                  <span className="chip chip-code">{s}</span>
                  <span className="secitem__name">{SECTION_NAMES[s] || "Раздел проектной документации"}</span>
                </div>
              ))}
            </div>
          </div>
  ...
  /* ============ DATE PICKER (нативный, в теме приложения) ============ */
  ...
  function DateField({ value, onChange, placeholder }) {
    ...
    const [vy, setVy] = React.useState((sel || today).getFullYear());
    const [vm, setVm] = React.useState((sel || today).getMonth());
    const ref = React.useRef(null);
    ...
    React.useEffect(() => {
      if (open) { const b = parseDate(value) || today; setVy(b.getFullYear()); setVm(b.getMonth()); setView("day"); }
        ...
        <div className="newgrid">
          <div className="card">
            ...
            {step === 1 && <div className="col gap18 fade-in">
              ...
              <div className="field"><label>Стадия проектирования</label>
                <div className="secpick secpick--amber">
                  {STAGE_OPTS.map(([code, name]) => {
                    const on = code === stage;
                    return (
                      <button key={code} type="button" className={"secpick__item" + (on ? " is-sel" : "")} onClick={() => setStage(code)}>
              ...
              <div className="field"><label>Тип привлечения</label><div className="row gap10 wrap pillpick--amber">{["Один специалист","Команда","Организация"].map((s) => <button key={s} type="button" className={"pill" + (s === attract ? " is-active" : "")} onClick={() => setAttract(s)}>{s}</button>)}</div></div>
            </div>}
            {step === 2 && <div className="col gap16 fade-in">
              <div className="row between" style={{ alignItems: "baseline" }}>
                <h3 className="section-title">Разделы документации</h3>
                <span className="dim" style={{ fontSize: 13 }}>Выбрано: {sel.length} из {allSections.length}</span>
              </div>
              <p className="muted" style={{ margin: 0, fontSize: 13.5 }}>Отметьте разделы проектной документации, которые нужно разработать.</p>
              <div className="secpick secpick--amber">
                {allSections.map(s => {
                  const on = sel.includes(s);
                  return (
                    <button key={s} type="button" className={"secpick__item" + (on ? " is-sel" : "")} onClick={() => toggle(s)}>
                      <span className="secpick__code">{s}</span>
                      <span className="secpick__name">{SECTION_NAMES[s] || "Раздел документации"}</span>
                      <span className="secpick__check"><Icon name="check" size={13} /></span>
                    </button>
                  );
                })}
            ...
            {step === 4 && <div className="col gap18 fade-in">
              <div className="field">
                ...
                <textarea className="input" rows={5} placeholder="Опишите задачу в свободной форме: особенности объекта, пожелания к решению, требования к исполнителю, важные сроки…" value={note} onChange={e => setNote(e.target.value)} />
              </div>
              <div className="field">
          ...
          <div className="card newgrid__preview prev">
            ...
            <div className="prev__cover">
              <window.Mascots scale={0.4} />
            </div>
