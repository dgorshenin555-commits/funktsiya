/* new_design.jsx — экран варианта Б, выгружен из основного проекта.
   Сгенерировано tools/reverse_transform.py: правки вносить в
   app/v2/_screens/ в проекте, иначе они разойдутся.
   Использует из lib_bundle.jsx: OBJECT_TYPE_LABELS, STAGE_LABELS, useApp.
...
(function () {
    ...
    { t: "Отклики", d: "Исполнители присылают предложения и условия.", c: (
      ...
      <div className="wc__list">
        {[["Техносфера", "ТС"], ["ИнжГрупп", "ИГ"], ["ПроектСтрой", "ПС"]].map(([n, ab]) => (
          <div key={n}>{n === "Техносфера" ? <span className="wc__logo wc__logo--s"><img src="assets/tehnosfera-logo.jpg" alt="" /></span> : <Init n={ab} size={22} />}<span>{n}</span><em>СРО</em></div>
        ))}
      </div>
      <div className="wc__link">Показать все отклики <Arr s={12} /></div></>) },
    { t: "Выбор исполнителя", d: "Вы сравниваете цену, срок и опыт.", c: (
      <><div className="wc__co"><span className="wc__logo"><img src="assets/tehnosfera-logo.jpg" alt="Техносфера" /></span><b>ООО «Техносфера»</b></div>
    ...
    return (
      <div className="scroll">
        ...
        <div className="hstrip">
          {[["assets/hero-industrial-2.png", "Промышленные объекты", "КЖ, ОВ, ЭОМ"], ["assets/hero-commercial-2.png", "Коммерция и склады", "Полный комплект ПИР"], ["assets/hero-private-2.png", "Жильё и благоустройство", "АР, ГП, ВК"]].map(([src, t, d]) => (
          ...
          <section className="statline">
            ...
            <div className="stat" style={{ marginTop: 0 }}>
              ...
              <div className="logos logos--inline">
                <span className="brand"><img src="assets/ridan.webp" alt="Ridan" /></span>
                <span className="brand"><img src="assets/caparol-logo.webp" alt="Caparol" style={{ height: 28 }} /></span>
                <span className="brand"><img src="assets/vandjord.jpg" alt="Vandjord" /></span>
                <span className="brand"><img src="assets/nemen.png" alt="Немен" /></span>
          ...
          <section className="shot">
            ...
            <div className="bento">
              ...
              <div className="bento__img">
                <img src="assets/hero-private.png" alt="" />
  ...
  function NewApp() {
    ...
    const SCR = {
      ...
      live: () => <window.LiveOrder go={go} orderId={liveId} />,
      track: () => <window.ReqTrack go={go} req={cur} />,
      detail: () => <window.OrderDetail go={go} />, order: () => <window.OrderCard go={go} />, prof: () => <window.ProProfile go={go} />,
      pick: () => <window.Pick go={go} />, solo: () => <window.SoloProfile go={go} />, cat: () => <window.Cat />,
      new: () => <window.OrderNew go={go} onPublish={publish} />, exp: () => <window.Expertise go={go} />,
      norm: () => <window.Standards />, msg: () => <window.Messages />, an: () => <window.Analytics />,
      price: () => <window.Pricing />, set: () => <window.Settings />,
      ai: () => <window.AiPick />,
    };
    return (
      <div className="nd" onClick={() => menu && setMenu(false)}>
        <header className="topbar">
          ...
          <img className="ava" src="assets/me.jpg" alt="" onClick={() => go("set")} style={{ cursor: "pointer" }} />
        </header>
        {(SCR[scr] || SCR.home)()}
      </div>
    );
