/* new_design.jsx — Функция 2026: paper/ink/acid restyle. Главная + Заявки */
(function () {
    ...
    { t: "Отклики", d: "Исполнители присылают предложения и условия.", c: (
      ...
      <div className="wc__list">
        {[["Техносфера", "ТС", 91], ["ИнжГрупп", "ИГ", 84], ["ПроектСтрой", "ПС", 78]].map(([n, ab, v]) => (
          <div key={n}>{n === "Техносфера" ? <span className="wc__logo wc__logo--s"><img src="assets/tehnosfera-logo.jpg" alt="" /></span> : <Init n={ab} size={22} />}<span>{n}</span><em className="num">{v}</em></div>
        ))}
      </div>
  ...
  function NewApp() {
    ...
    const SCR = {
      ...
      trust: () => <window.Trust />, price: () => <window.Pricing />, set: () => <window.Settings />,
      ai: () => <window.AiPick />,
    };
    return (
